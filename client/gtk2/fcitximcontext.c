...
static const uint32_t purpose_related_capacity =
    CAPABILITY_ALPHA |
    CAPABILITY_DIGIT |
    CAPABILITY_NUMBER |
    CAPABILITY_DIALABLE |
...
static const uint32_t hints_related_capacity =
    CAPABILITY_SPELLCHECK |
    CAPABILITY_NO_SPELLCHECK |
    CAPABILITY_WORD_COMPLETION |
    CAPABILITY_LOWERCASE |
    CAPABILITY_UPPERCASE |
    CAPABILITY_UPPERCASE_WORDS |
    CAPABILITY_UPPERCASE_SENTENCES |
    CAPABILITY_NO_ON_SCREEN_KEYBOARD;
...
struct _FcitxIMContext {
    ...
    int cursor_pos;
    FcitxCapabilityFlags capacity_from_toolkit;
    FcitxCapabilityFlags last_updated_capacity;
...
static void
_fcitx_im_context_forward_key_cb(FcitxClient* client, guint keyval, guint state, gboolean isRelease, void* user_data);
static void
...
fcitx_im_context_init(FcitxIMContext *context)
{
    ...
    context->last_updated_capacity = CAPABILITY_SURROUNDING_TEXT;
                                  ...
                                  gint cursor_index)
{
    ...
    if (fcitx_client_is_valid(fcitxcontext->client) &&
        !(fcitxcontext->last_updated_capacity & CAPABILITY_PASSWORD)) {
        gint cursor_pos;
        guint utf8_len;
        gchar *p;
...
{
    if (fcitx_client_is_valid(fcitxcontext->client)) {
        FcitxCapabilityFlags flags = fcitxcontext->capacity_from_toolkit;
        // toolkit hint always not have preedit / surrounding hint
        // no need to check them
        if (fcitxcontext->use_preedit) {
            flags |= CAPABILITY_PREEDIT | CAPABILITY_FORMATTED_PREEDIT;
        }
        if (fcitxcontext->support_surrounding_text) {
            flags |= CAPABILITY_SURROUNDING_TEXT;
        }
...
void _fcitx_im_context_forward_key_cb(FcitxClient* im, guint keyval, guint state, gboolean isRelease, void* user_data)
{
    FCITX_UNUSED(im);
                  ...
                  bool isRelease)
{
    ...
    GdkEventKey *event = (GdkEventKey*)gdk_event_new(isRelease ? GDK_KEY_RELEASE : GDK_KEY_PRESS);
...
#define CASE_PURPOSE(_PURPOSE, _CAPACITY) \
    ...
    switch(purpose) {
        CASE_PURPOSE(GTK_INPUT_PURPOSE_ALPHA, CAPABILITY_ALPHA)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_DIGITS, CAPABILITY_DIGIT);
        CASE_PURPOSE(GTK_INPUT_PURPOSE_NUMBER, CAPABILITY_NUMBER)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_PHONE, CAPABILITY_DIALABLE)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_URL, CAPABILITY_URL)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_EMAIL, CAPABILITY_EMAIL)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_NAME, CAPABILITY_NAME)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_PASSWORD, CAPABILITY_PASSWORD)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_PIN, CAPABILITY_PASSWORD | CAPABILITY_DIGIT)
...
#define CHECK_HINTS(_HINTS, _CAPACITY) \
    ...
    CHECK_HINTS(GTK_INPUT_HINT_SPELLCHECK, CAPABILITY_SPELLCHECK)
    CHECK_HINTS(GTK_INPUT_HINT_NO_SPELLCHECK, CAPABILITY_NO_SPELLCHECK);
    CHECK_HINTS(GTK_INPUT_HINT_WORD_COMPLETION, CAPABILITY_WORD_COMPLETION)
    CHECK_HINTS(GTK_INPUT_HINT_LOWERCASE, CAPABILITY_LOWERCASE)
    CHECK_HINTS(GTK_INPUT_HINT_UPPERCASE_CHARS, CAPABILITY_UPPERCASE)
    CHECK_HINTS(GTK_INPUT_HINT_UPPERCASE_WORDS, CAPABILITY_UPPERCASE_WORDS)
    CHECK_HINTS(GTK_INPUT_HINT_UPPERCASE_SENTENCES, CAPABILITY_UPPERCASE_SENTENCES)
    CHECK_HINTS(GTK_INPUT_HINT_INHIBIT_OSK, CAPABILITY_NO_ON_SCREEN_KEYBOARD)
