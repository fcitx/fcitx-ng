...
void compile_to_c(FcitxConfiguration* config, FcitxStringHashSet* structs, const char* name, const char* prefix)
{
...
{
    int c;
    FcitxConfigDescCompileOperation op = Operation_C;
    char* name = NULL;
    char* prefix = NULL;
    while ((c = getopt(argc, argv, "n:p:th")) != EOF) {
        switch (c) {
            case 'n':
    ...
    switch(op) {
        case Operation_C:
            if (name) {
                compile_to_c(config, structs, name, prefix ? prefix : "");
            }
            break;
