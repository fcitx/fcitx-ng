...
int main()
{
    ...
    FcitxStandardPathFile* files = fcitx_standard_path_locate(sp, FSPT_Config, "fcitx5/config", 0);
    fcitx_standard_path_file_close(files);
    FcitxStandardPathFilter filter;
    filter.flag = FSPFT_Suffix | FSPFT_Sort;
    filter.suffix = ".conf";
    FcitxDict* fileDict = fcitx_standard_path_match(sp, FSPT_Data, "fcitx5/addon", &filter);
